...
export default function WorkspaceSnippetPanel({
  ...
  const selected =
    snippets.find((item) => item.snippet.id === selectedId)?.snippet ??
    snippets[0]?.snippet ??
    null
  ...
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] overflow-hidden min-h-[620px]">
      <div className="flex h-[calc(100vh-260px)] min-h-[620px]">
        <aside className="w-[320px] shrink-0 border-r border-[var(--border)] flex flex-col">
        ...
        <section className="flex-1 min-w-0 flex flex-col">
          {selected ? (
            <>
              <div className="px-5 py-3 border-b border-[var(--border)] bg-[var(--surface)] flex items-center justify-between gap-3 shrink-0">
                <div>
              ...
              <div className="flex-1 min-h-0">
                <SnippetDetail
                  ...
                  snippet={selected}
                  onEdit={() => {
                    // nanti kita sambungkan ke edit modal / route edit workspace
                  }}
                />
              </div>
            </>
